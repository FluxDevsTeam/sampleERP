    ...
    const fetchData = async () => {
      ...
      try {
        ...
        if (data?.categorical_data_year?.expense_categories && data?.categorical_data_month?.expense_categories) {
          ...
          const yearSum = yearCategories.reduce((sum, item) => sum + item.value, 0);
          const monthSum = monthCategories.reduce((sum, item) => sum + item.value, 0);
          
          // Add percentage to each item
          const processedYearData = yearCategories.map(item => ({
            ...item,
            percent: item.value / yearSum
          })).sort((a, b) => b.value - a.value); // Sort by value descending
          
          const processedMonthData = monthCategories.map(item => ({
            ...item,
  ...
  if (loading) {
    return (
      <div className="flex justify-center items-center h-64">
        <div className="animate-pulse text-blue-600">Loading expense data...</div>
      </div>
    );
  }
      ...
      <div className="bg-white rounded-xl shadow-lg overflow-hidden">
        ...
        <div className="p-4 bg-[#f5f7fa]">
          ...
          <div className="mt-4 border-t border-gray-100 pt-4">
            <h3 className="font-medium text-gray-700 mb-2">Top Expense Categories</h3>
            <div className="space-y-2">
              {yearData.slice(0, 3).map((category, index) => (
                <div key={index} className="flex justify-between items-center">
                  ...
                  <span className="font-medium">{formatValue(category.value)}</span>
                </div>
              ))}
            </div>
          </div>
