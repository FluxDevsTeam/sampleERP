...
interface TableData {
  Category: string;
  "Total Cost Value": number | string;
  "Total Profit": number | string;
  "Total Stock Value": number | string;
...
const CategoryTable: React.FC<TableProps> = ({ headers }) => {
  ...
  useEffect(() => {
    const fetchItems = () => {
      try {
        setLoading(true);
        const updatedTableData: TableData[] = dashboardDataJson.shop_category_data.map(
          (item: any) => ({
        ...
        {loading ? (
          ...
          <div className="flex flex-col items-center justify-center py-6 bg-white rounded-lg border border-gray-200 shadow-sm mb-10">
            ...
            <p className="text-gray-500 mb-6 text-center max-w-xs">
              All your inventory categories will show up here. Add a new category to get started.
            </p>
          </div>
        ) : (
          <div>
